...
export default {
  ...
  getters: {
    ...
    sortedFriends: (state) => (sortAsc) => {
      return [...state.friends].sort(
        (a, b) =>
          (a[1].shadowpayUserId - b[1].shadowpayUserId) * (sortAsc ? 1 : -1),
      );
    },
  ...
  actions: {
    async loadFriends({ rootState, commit }) {
      ...
      for (let i = 0; i < loopLimit; i++) {
        const { success, data } = await userFriend.all(
    ...
    async addFriend({ rootState, commit, dispatch }, friend) {
      const { success, data, error_message } = await userFriend.create(
        rootState.session.token,
        friend,
      );
    ...
    async updateFriend({ rootState, commit, dispatch }, friend) {
      const { success, data, error_message } = await userFriend.update(
        rootState.session.token,
        friend,
      );
    ...
    async deleteFriend({ rootState, commit, dispatch }, id) {
      const { success, data, error_message } = await userFriend.remove(
        rootState.session.token,
        id,
      );
