  ...
  mutations: {
    ...
    setItemMarketPrice(state, { id, price }) {
      ...
      item.item.discount =
        100 -
        Math.round(
          (item.item.price_market_usd * 100) / item.item.steam_price_en,
        );
    },
  },
  actions: {
    async startTrack({ rootState, commit, dispatch }, metadata) {
      const { success, data, error_message } = await saleGuardItem.create(
    ...
    async updateTracked({ rootState, dispatch }, item) {
      const { success, error_message } = await saleGuardItem.update(
        rootState.session.token,
        item,
      );
    ...
    async adjustTracked({ rootGetters, getters, commit, dispatch }) {
      for (const { item, metadata } of getters.trackedItems()) {
        ...
        const minPrice = round(
          rootGetters["app/config"]("saleGuardLowerLimit") * marketData.price,
        );
        const maxPrice = round(
          rootGetters["app/config"]("saleGuardUpperLimit") * marketData.price,
        );
    ...
    async stopTrack({ rootState, commit, dispatch }, { id, showAlert = true }) {
      const { success, data, error_message } = await saleGuardItem.remove(
        rootState.session.token,
        id,
      );
    ...
    async loadSaleGuardItems({ state, rootState, commit }) {
      ...
      for (let i = 0; i < loopLimit; i++) {
        const { success, data } = await saleGuardItem.all(
          rootState.session.token,
