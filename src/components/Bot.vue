...
export default {
  ...
  methods: {
    ...
    async buyItem(item) {
      if (this.items.pending.has(item.id) || item.price_market_usd + this.moneySpent > this.preset.toSpend) {
        return
      }
    ...
    updatePending() {
      if (this.items.pending.size === 0 || Date.now() - this.lastPendingUpdate < this.pendingUpdateDelay * 1000) {
        return
      }
