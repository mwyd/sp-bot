...
export default function ({ baseUrl, service }) {
  ...
  const create = (token, { shadowpayUserId, name }) => fetchBackground({
    service,
    data: {
      ...
      config: {
        ...
        body: `shadowpay_user_id=${shadowpayUserId}`
          + `&name=${name}`
      }
    }
  })

  const update = (token, { id, friend }) => fetchBackground({
    service,
    data: {
      ...
      config: {
        ...
        body: `shadowpay_user_id=${friend.shadowpayUserId}`
          + `&name=${friend.name}`
      }
    }
  })
