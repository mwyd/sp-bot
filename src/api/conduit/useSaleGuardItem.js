...
export default function ({ baseUrl, service }) {
  ...
  const create = (token, { shadowpayOfferId, hashName, minPrice, maxPrice }) => fetchBackground({
    service,
    data: {
      ...
      config: {
        ...
        body: `shadowpay_offer_id=${shadowpayOfferId}`
          + `&hash_name=${hashName}`
          + `&min_price=${minPrice}`
          + `&max_price=${maxPrice}`
      }
  ...
  const update = (token, { id, data }) => fetchBackground({
    service,
    data: {
      ...
      config: {
        ...
        body: `shadowpay_offer_id=${data.shadowpayOfferId}`
